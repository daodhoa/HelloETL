...
public class ExcelUltils {
    ...
    public ArrayList<String> getColumnName(int indexSheet, String filePath) throws IOException {
        ArrayList<String> columnNames = new ArrayList<>();
        Workbook workbook = this.readExcel(filePath);
        Sheet sheet = workbook.getSheetAt(indexSheet);
        Row firstRows = sheet.getRow(0);
        Iterator<Cell> cellIterator = firstRows.iterator();
        while (cellIterator.hasNext()) {
            Cell cell = cellIterator.next();
            String cellValue = dataFormatter.formatCellValue(cell);
